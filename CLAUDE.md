...
- **Shared email shell (both emails)** — keep these in sync between `supabase/templates/confirm-signup.html` and `buildEmailHtml` in `notify-message`; they intentionally use the same header/footer markup:
  - **Header logo**: `https://runimarket.org/og-image.png` at **56×56**, sized in *both* the HTML attributes and the inline CSS, plus `border="0"`, `outline:none`, `text-decoration:none` and `-ms-interpolation-mode:bicubic` (Outlook downscales the 512px source badly without it). Centered via `display:block;margin:0 auto 10px auto` inside a `background-color:#102C97` cell — the og-image tile blue matches the header, so the star crescent blends in seamlessly.
  - **No em-dashes in email copy** (deliberate, user request). The footer reads "Student-run side project**,** not an official university initiative." Do not "restore" the em-dash when editing these two files. Em-dashes in *code comments* are fine — they never reach a recipient.
- ⚠️ **Email previews lie about images.** The Supabase "Confirm signup" preview pane renders in a sandboxed iframe that blocks external images, so the logo shows as a broken-image icon there even when it is perfectly fine; Outlook/Gmail do the same until the user clicks "Show blocked content"/"Display images". Verify the logo by opening `https://runimarket.org/og-image.png` directly (should be a blue tile with the white star crescent) — **not** by trusting the preview pane or a screenshot of a client with images blocked.
- ⚠️ Deploy reminder: pushing to main does NOT deploy edge fns — if `notify-message` code changes, redeploy it via Dashboard/CLI. All three edge fns (`import-event`, `translate-listing`, `notify-message`) currently show as deployed. Same for `confirm-signup.html`: it is **not** read from the repo, it must be re-pasted into Dashboard → Auth → Email Templates → "Confirm signup" after every edit.
